        ...
        with open(self.source_file, 'r') as f:
            for line_num, line in enumerate(f.readlines(), 1):
                ...
                if stripped and not stripped.startswith('--') and \
                   any(keyword in stripped for keyword in ['<=', ':=', 'assign', 'when', 'if', 'case']):
...
def generate_html_report(parser, output_file='coverage_detail.html'):
    ...
    uncovered_lines = sum(1 for line in parser.lines_data if line['status'] == 'uncovered')
...
if __name__ == "__main__":
    ...
    parser_args = argparse.ArgumentParser(description='Generate coverage report for AOI 2-2')
    parser_args.add_argument('--sim', choices=['verilator', 'ghdl', 'auto'], 
                           default='auto',
                           help='Specify simulator (default: auto-detect)')
    parser_args.add_argument('--source', type=str,
                           help='Source file path (overrides auto-detection)')
    ...
    if simulator == "verilator":
        ...
        print(f"Unknown simulator, using basic parser")
        parser = GHDLCoverageParser(source_file)
